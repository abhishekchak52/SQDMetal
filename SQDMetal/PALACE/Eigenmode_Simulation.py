...
class PALACE_Eigenmode_Simulation(PALACE_Model_RF_Base):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for key in PALACE_Eigenmode_Simulation.default_user_options:
            self.user_options[key] = user_options.get(
                key, PALACE_Eigenmode_Simulation.default_user_options[key]
            )
    ...
    def create_config_file(self, **kwargs):
        ...
        if self.meshing == "COMSOL":
            ...
            material_air = list(
                comsol._model.java.component("comp1")
                .material("Vacuum")
                .selection()
                .entities()
            )
            material_dielectric = list(
                comsol._model.java.component("comp1")
                .material("Substrate")
                .selection()
                .entities()
            ...
            for m, cur_port in enumerate(self._ports):
                if cur_port["elem_type"] == "cpw":
                    ports[cur_port["port_name"] + "a"] = list(
                        comsol._model.java.component("comp1")
                        .physics("emw")
                        .feature(f"lport{m}")
                        .feature("ue2")
                    ...
                    ports[cur_port["port_name"] + "b"] = list(
                        comsol._model.java.component("comp1")
                        .physics("emw")
                        .feature(f"lport{m}")
                        .feature("ue1")
        ...
        config = {
            ...
            "Solver": {
                ...
                "Eigenmode": {
                    ...
                    / 1e9,  # GHz - starting point
                    "Save": self.user_options[
                        "solns_to_save"
                    ],  # Number of computed field modes to save to disk for visualization with ParaView
                },
                "Linear": {
                    "Type": "SuperLU",
                    "KSPType": "FGMRES",
        ...
        if os.path.exists(lePlots):
            ...
            for m in range(leView.num_datasets):
                try:
                    ...
                    fig = leSlice.plot(
                        np.linalg.norm(leSlice.get_data("E_real"), axis=1),
                        "coolwarm",
                        True,
                    )
            ...
            if self.meshing == "GMSH":
                GMSH_Navigator(self._mesh_path).export_to_png(
                    file_path=self._output_data_dir + "/mesh.png"
                )
            else:
                try:
    ...
    def retrieve_EPR_data(self):
        return PALACE_Eigenmode_Simulation.retrieve_EPR_data_from_file(
            self._sim_config, self._output_data_dir
        )

    def retrieve_mode_port_EPR(self):
        return PALACE_Eigenmode_Simulation.retrieve_mode_port_EPR_from_file(
    ...
    def retrieve_EPR_data_from_file(json_sim_config, output_directory):
        ...
        with open(json_sim_config, "r") as f:
            config_json = json.loads(f.read())
        assert "Boundaries" in config_json, (
            '"Boundaries" not found in the JSON configuration.'
        )
        leDict = config_json["Boundaries"]
        assert "Postprocessing" in leDict, (
            '"Postprocessing" not found in the JSON configuration.'
        )
        leDict = leDict["Postprocessing"]
        assert "Dielectric" in leDict, (
            '"Dielectric" not found in the JSON configuration.'
        )
        ...
        raw_data = raw_data.to_numpy()
        col_Ref = [
            x for x in range(len(headers)) if headers[x].strip().startswith(r"Re{f}")
        ][0]
        col_Ief = [
            x for x in range(len(headers)) if headers[x].strip().startswith(r"Im{f}")
        ][0]
        col_Q = [x for x in range(len(headers)) if headers[x].strip().startswith(r"Q")][
            0
        ]
        ...
        for m, cur_row in enumerate(raw_data):
            ...
            for cur_interface in epr_dict:
                ...
                cur_mode[cur_interface]["p"] = raw_dataEPR[
                    m, 2 * epr_dict[cur_interface] - 1
                ]
                cur_mode[cur_interface]["Q"] = raw_dataEPR[
                    m, 2 * epr_dict[cur_interface]
                ]
    ...
    def retrieve_mode_port_EPR_from_file(output_directory):
        ...
        return {
            ...
            "eigenfrequencies": raw_dataE[:, col_Ref] * 1e9,
        }
    ...
    def calculate_hamiltonian_parameters_EPR(directory, modes_to_compare=[]):
        ...
        if modes_to_delete.size > 0:
            for i, mode in enumerate(modes_to_delete):
                participation_ratios = np.delete(
                    participation_ratios, (modes_to_delete[i] - (i + 1)), axis=0
                )
                frequencies = np.delete(
                    frequencies, (modes_to_delete[i] - (i + 1)), axis=0
                )
        ...
        # Detuning matrix
        delta = np.ones(
            (np.shape(omega)[0], np.shape(omega)[0])
        )  # create delta/detuning matrix
        ...
        diag_matrix = (-1 / 2) * np.diag(np.ones(num_of_modes)) + np.ones(
            num_of_modes
        )  # hacky way of creating ones matrix with values of 1/2 down the diagonal
        chi_anharm = np.multiply(
            diag_matrix, chi
        )  # This is the kerr matrix but with the anharmonicity down the diagonal for the self-kerr term
        anharm = (1 / 2) * np.diagonal(chi)
        lamb_shift = (1 / 2) * chi * np.ones((np.shape(chi)[0], 1))
        renormalised_freqs = (
            np.transpose(np.matrix(frequencies * 2 * np.pi)) - lamb_shift
        )  # linear frequencies from eigenmode simulation are renormalised by lamb shift caused by non-linearity
        ...
        freq_df = pd.DataFrame(
            frequencies / (1e9), dataframe_label_mode, ["Freq (GHz)"]
        )
        print(freq_df)
        ...
        freq_renorm_df = pd.DataFrame(
            renormalised_freqs, dataframe_label_mode, ["Freq (GHz)"]
        )
        ...
        ratios_df = pd.DataFrame(
            np.abs(participation_ratios), dataframe_label_mode, dataframe_label_junc
        )
        print(ratios_df)
        print("______________________________")
        print("\n")

        chi_anharm_df = pd.DataFrame(
            chi_anharm, dataframe_label_mode, dataframe_label_mode
        )
        ...
        lamb_shift_df = pd.DataFrame(
            lamb_shift_freq, dataframe_label_mode, ["Lamb Shift (MHz)"]
        )
        print("Lamb Shifts:")
        print(lamb_shift_df)
        print("______________________________")

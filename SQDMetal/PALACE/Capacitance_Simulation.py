...
class PALACE_Capacitance_Simulation(PALACE_Model):
        ...
        **kwargs,
        ...
        for key in PALACE_Capacitance_Simulation.default_user_options:
            self.user_options[key] = user_options.get(
                key, PALACE_Capacitance_Simulation.default_user_options[key]
            )
    ...
    def display_conductor_indices(self):
        ...
        """
        assert len(self._cur_cap_terminals) > 0, (
            "There are no terminals. Ensure prepare_simulation() has been called."
        )

        minX = (
            self._geom_processor.chip_centre[0] - self._geom_processor.chip_size_x * 0.5
        )
        maxX = (
            self._geom_processor.chip_centre[0] + self._geom_processor.chip_size_x * 0.5
        )
        minY = (
            self._geom_processor.chip_centre[1] - self._geom_processor.chip_size_y * 0.5
        )
        maxY = (
            self._geom_processor.chip_centre[1] + self._geom_processor.chip_size_y * 0.5
        )
        chip_bounding_poly = shapely.LineString(
            [(minX, minY), (maxX, minY), (maxX, maxY), (minX, maxY), (minX, minY)]
        )

        leGeoms = [chip_bounding_poly] + self._cur_cap_terminals
        leNames = ["Chip"] + [
            f"Cond{x + 1}" for x in range(len(self._cur_cap_terminals))
        ]
        gdf = gpd.GeoDataFrame({"names": leNames}, geometry=leGeoms)
        fig, ax = plt.subplots(1)
        gdf.plot(
            ax=ax, column="names", cmap="jet", alpha=0.5, categorical=True, legend=True
        )
    ...
    def create_config_file(self, **kwargs):
        ...
        if self.meshing == "GMSH":
            ...
            for i, value in enumerate(gmsh_render_attrs["metals"]):
                metal = {"Index": i + 1, "Attributes": [value]}
        ...
        if self.meshing == "COMSOL":
            ...
            for i, term_name in enumerate(sCapMats.terminal_names):
                ...
                metal = {"Index": i + 1, "Attributes": term_assigned_value}
                Terminal.append(metal)
        ...
        for x in Terminal:
            post_procs.append(
                {"Index": x["Index"], "Attributes": x["Attributes"], "Type": "Electric"}
            )
        config = {
            "Problem": {
                "Type": "Electrostatic",
            ...
            "Solver": {
                ...
                "Electrostatic": {
                    "Save": len(Terminal)
                    if self.user_options["solns_to_save"] == -1
                    else self.user_options["solns_to_save"]
                },
                "Linear": {
                    "Type": "BoomerAMG",
                    "KSPType": "CG",

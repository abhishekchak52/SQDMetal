...
class PALACE_Model:
    def __init__(self, meshing, mode, options):
        ...
        if (
            isinstance(restrict_rect, list)
            or isinstance(restrict_rect, np.ndarray)
            or isinstance(restrict_rect, tuple)
        ):
            self.restrict_rect = [
                min([restrict_rect[0], restrict_rect[2]]),
                min([restrict_rect[1], restrict_rect[3]]),
                max([restrict_rect[0], restrict_rect[2]]),
                max([restrict_rect[1], restrict_rect[3]]),
            ]
            self.chip_len = self.restrict_rect[2] - self.restrict_rect[0]
            self.chip_wid = self.restrict_rect[3] - self.restrict_rect[1]
            self.chip_centre = [
                (self.restrict_rect[0] + self.restrict_rect[2]) * 0.5,
                (self.restrict_rect[1] + self.restrict_rect[3]) * 0.5,
                QUtilities.parse_value_length(
                    self.metal_design.chips["main"]["size"]["center_z"]
                ),
            ]
        else:
            self.chip_len = QUtilities.parse_value_length(
                self.metal_design.chips["main"]["size"]["size_x"]
            )
            self.chip_wid = QUtilities.parse_value_length(
                self.metal_design.chips["main"]["size"]["size_y"]
            )
            self.chip_centre = [
                QUtilities.parse_value_length(
                    self.metal_design.chips["main"]["size"][x]
                )
                for x in ["center_x", "center_y", "center_z"]
            ]
            self.restrict_rect = [
                self.chip_centre[0] - 0.5 * self.chip_len,
                self.chip_centre[1] - 0.5 * self.chip_wid,
                self.chip_centre[0] + 0.5 * self.chip_len,
                self.chip_centre[1] + 0.5 * self.chip_wid,
            ]
    ...
    def fine_mesh_along_path(self, dist_resolution, qObjName, trace_name="", **kwargs):
        ...
        lePath = (
            QUtilities.calc_points_on_path(
                dist_resolution / leUnits, self.metal_design, qObjName, trace_name
            )[0]
            * leUnits
        )
...
class PALACE_Model_RF_Base(PALACE_Model):
    ...
    def create_port_CPW_on_Launcher(
        ...
        launchesA, launchesB, vec_perp = QUtilities.get_RFport_CPW_coords_Launcher(
            self.metal_design, qObjName, len_launch, 1
        )  # Units of m...
    ...
    def create_port_CPW_on_Route(
        ...
        launchesA, launchesB, vec_perp = QUtilities.get_RFport_CPW_coords_Route(
            self.metal_design, qObjName, pin_name, len_launch, 1
        )  # Units of m...

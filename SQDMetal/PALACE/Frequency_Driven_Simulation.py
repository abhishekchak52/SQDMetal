...
class PALACE_Driven_Simulation(PALACE_Model_RF_Base):
        ...
        **kwargs,
        ...
        for key in PALACE_Driven_Simulation.default_user_options:
            self.user_options[key] = user_options.get(
                key, PALACE_Driven_Simulation.default_user_options[key]
            )
    ...
    def create_config_file(self, **kwargs):
        ...
        config = {
            "Problem": {"Type": "Driven", "Verbose": 2, "Output": self._output_dir},
            "Model": {
            ...
            "Solver": {
                ...
                "Driven": {
                    ...
                    "SaveStep": self.user_options[
                        "solns_to_save"
                    ],  # Number of frequency steps for computed field modes to save to disk for visualization with ParaView
                },
                "Linear": {
                    "Type": "SuperLU",
                    "KSPType": "FGMRES",
    ...
    def add_surface_current_source_region(self, region="", direction=1):
        assert region in ["metals", "dielectric"], (
            "Must set region to either 'metals' or 'dielectric'"
        )
        assert direction == -1 or direction == 1, (
            "Direction must be either 1 or -1 for +z or -z"
        )
